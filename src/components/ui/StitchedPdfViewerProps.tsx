...
export default function StitchedPdfViewer({
  ...
  const loadPdfJs = async () => {
    ...
    return new Promise((resolve, reject) => {
      ...
      script.onerror = () => reject(new Error('خطأ في تحميل مكتبة PDF.js'));
  ...
  const convertPdfToLongImage = async (pdfFile: File) => {
    ...
    try {
      ...
      if (onLoaded) {
        onLoaded({ width: maxWidth, height: totalHeight, pageCount: numPages, imageUrl: dataUrl, pageHeights: pageImages.map(c => c.height) });
      }
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center p-8">
        ...
        <p className="text-gray-700 font-medium">جارى معالجه الـ PDF</p>
