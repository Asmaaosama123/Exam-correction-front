...
export function useLogin() {
  ...
  return useMutation({
    ...
    onSuccess: ({ loginResponse, userResponse }) => {
      ...
      const userData = {
        ...
        roles: userResponse.roles || loginResponse.roles || [],
        maxAllowedPages: userResponse.maxAllowedPages || 0,
        usedPages: userResponse.usedPages || 0,
        isSubscribed: userResponse.isSubscribed || false,
        subscriptionExpiryUtc: userResponse.subscriptionExpiryUtc || null,
        isSubscriptionModeEnabled: userResponse.isSubscriptionModeEnabled ?? true,
      ...
      const isAdminUser = userResponse.roles?.some((role: string) => role.toLowerCase() === "admin");
      const isAdminLogin = loginResponse.roles?.some((role: string) => role.toLowerCase() === "admin");
      const isAdmin = isAdminUser || isAdminLogin;
      
      const isAITrainerUser = userResponse.roles?.some((role: string) => role.toLowerCase() === "aitrainer");
      const isAITrainerLogin = loginResponse.roles?.some((role: string) => role.toLowerCase() === "aitrainer");
      const isAITrainer = isAITrainerUser || isAITrainerLogin;

      let returnUrl = "/dashboard";
      if (isAdmin) returnUrl = "/admin";
      else if (isAITrainer) returnUrl = "/ai-dashboard";
    ...
    queryFn: async () => {
      ...
      try {
        ...
        return {
          ...
          maxAllowedPages: response.maxAllowedPages || 0,
          usedPages: response.usedPages || 0,
          isSubscribed: response.isSubscribed || false,
          subscriptionExpiryUtc: response.subscriptionExpiryUtc || null,
          isSubscriptionModeEnabled: response.isSubscriptionModeEnabled ?? true,
          isAuthenticated: true,
        };
      } catch (error) {
