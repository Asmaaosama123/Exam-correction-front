...
api.interceptors.response.use(
  ...
  async (error: AxiosError<ApiErrorResponse>) => {
    ...
    if (!error.response) {
      try {
        if (error.config?.url && !error.config.url.includes('/api/AdminLogs/client-error')) {
          axios.post(API_BASE_URL + "/AdminLogs/client-error", {
            errorMessage: `Network Error on ${error.config?.method?.toUpperCase()} ${error.config?.url}`,
            errorDetails: error.message,
            errorSource: "FRONTEND_NETWORK"
          }, {
            headers: error.config?.headers?.Authorization ? { Authorization: error.config.headers.Authorization } : {}
          }).catch(() => { });
        }
      } catch (e) { }
    ...
    try {
      if (error.config?.url && !error.config.url.includes('/api/AdminLogs/client-error')) {
        axios.post(API_BASE_URL + "/AdminLogs/client-error", {
          errorMessage: `API Error ${error.response.status} on ${error.config?.method?.toUpperCase()} ${error.config?.url}`,
          errorDetails: typeof errorResponse === 'object' ? JSON.stringify(errorResponse) : String(errorResponse),
          errorSource: "FRONTEND_HTTP"
        }, {
          headers: error.config?.headers?.Authorization ? { Authorization: error.config.headers.Authorization } : {}
        }).catch(() => { });
      }
...
// Helper function to extract Error Message for Display to users
export const getErrorMessage = (error: unknown): string => {
  const apiError = extractApiError(error);

  // 1. Try to extract specific messages from our API (Result.Failure/Error structure)
  if (apiError) {
    ...
    // Handle single Error record from our API structure (Code/Description)
    const detail = (apiError as any).description || (apiError as any).Description || (apiError as any).detail;
    if (detail) {
      return detail;
    }

    // Handle legacy format (Array of errors)
    if (isLegacyErrorResponse(apiError) && apiError.errors.length > 0) {
      const firstErr = apiError.errors[0];
      return (firstErr as any).description || (firstErr as any).Description || (firstErr as any).title || "";
    }

    // Fallback to title from ProblemDetails
    if (apiError.title && !apiError.title.toLowerCase().includes("bad request") && !apiError.title.toLowerCase().includes("server error")) {
      return apiError.title;
    }
  }

  // 2. Map HTTP Status Codes to Friendly Arabic messages if no specific detail is found
  if (axios.isAxiosError(error)) {
    const status = error.response?.status;

    switch (status) {
      case 400:
        return "هناك خطأ في البيانات المدخلة، يرجى مراجعتها والمحاولة مرة أخرى.";
      case 401:
        return "انتهت الجلسة أو ليس لديك صلاحية، يرجى تسجيل الدخول مرة أخرى.";
      case 403:
        return "عذراً، ليس لديك الصلاحية للقيام بهذا الإجراء.";
      case 404:
        return "المورد المطلوب غير موجود، قد يكون قد تم حذفه أو نقله.";
      case 409:
        return "هذا السجل موجود بالفعل، يرجى التأكد من البيانات.";
      case 422:
        return "تعذر معالجة البيانات، يرجى التأكد من صحة الحقول.";
      case 429:
        return "لقد قمت بإرسال الكثير من الطلبات، يرجى الانتظار قليلاً المحاولة مجدداً.";
      case 500:
      case 502:
      case 503:
      case 504:
        return "عذراً، حدثت مشكلة تقنية في السيرفر. جاري العمل على حلها حالياً.";
    }

    // Connection errors
    if (error.code === 'ECONNREFUSED' || error.message === 'Network Error') {
      return "فشل الاتصال بالسيرفر، يرجى التأكد من اتصال الإنترنت أو محاولة الوصول لاحقاً.";
    }

    if (error.code === 'ETIMEDOUT' || error.message.includes('timeout')) {
      return "استغرق الطلب وقتاً أطول من اللازم، يرجى المحاولة مرة أخرى.";
    }
  }

  // 3. Fallback for generic Errors or Unknown cases
  if (error instanceof Error) {
    return error.message;
  }

  return "حدث خطأ غير متوقع. يرجى مراجعة البيانات والمحاولة مرة أخرى.";
};
