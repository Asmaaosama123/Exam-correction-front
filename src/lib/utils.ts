...
/**
 * Format date in Arabic format (Only YYYY-MM-DD) with Arabic numerals
 * @param date - Date object to format
 * @returns Formatted date string in Arabic (YYYY-MM-DD)
 */
export function formatArabicDateOnly(date: Date = new Date()): string {
  const year = date.getFullYear();
  const month = date.getMonth() + 1;
  const day = date.getDate();
